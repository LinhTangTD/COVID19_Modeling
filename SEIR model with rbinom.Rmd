                    ...
                    for(p.sym.v in p.sym.v.list){
                      for(p.tp.sym.all in p.tp.sym.all.list){
                        for(p.tp.asym.all in p.tp.asym.all.list){
                          for(i in num_sim){
                            cur_cond_data = model(H.rate.nv, H.rate.v, p.fp.all, p.test.sym.nv, p.test.asym.nv, p.test.sym.v, p.test.asym.v,
                                                I.rate.sym, I.rate.asym, p.sym.nv, p.sym.v, p.tp.sym.all, p.tp.asym.all, num_day)
                            Peak.Infected = max(cur_cond_data$Newly.Infected.All)
                            Total.Infected = cur_cond_data$Total.Infected.So.Far[num_day]
                            Peak.Hospitalized = max(cur_cond_data$Current.Hospitalized.All)
                            Total.Dead = cur_cond_data$Total.Dead.So.Far[num_day]
                            all_data[row,] = list(H.rate.nv, H.rate.v, p.fp.all, p.test.sym.nv, p.test.asym.nv, p.test.sym.v, p.test.asym.v,
                                                 I.rate.sym, I.rate.asym, p.sym.nv, p.sym.v, p.tp.sym.all, p.tp.asym.all, 
                                                 Peak.Infected, Total.Infected, Peak.Hospitalized, Total.Dead)
                            row = row + 1
                          }
                        }
                      }
